...
@interface MusicAlbumAnimationView()
...
@implementation MusicAlbumAnimationView

- (instancetype)init{
    return [self initWithFrame:CGRectMake(0, 0,45,45)];
}
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if(self){
        ...
        backgroundLayer.contents = (id)[UIImage imageNamed:@"music_cover"].CGImage;
        
        [_albumContainer.layer addSublayer:backgroundLayer];
        _AlbumImage = [[UIImageView alloc]initWithFrame:CGRectMake(15, 15, 20, 20)];
        _AlbumImage.contentMode = UIViewContentModeScaleAspectFill;
        [_albumContainer addSubview:_AlbumImage];
    }
    return self;
}

- (void)startAnimation:(CGFloat)rate{
    NSLog(@"startAnimation %f",rate);
    rate = rate <= 0 ? 15 : rate;
    [self resetView];
