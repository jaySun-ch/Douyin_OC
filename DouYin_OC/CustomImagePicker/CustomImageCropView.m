...
@interface CustomImageCropView()<UIScrollViewDelegate>
...
@implementation CustomImageCropView
...
-(void)SetImage{
    ...
    [self.scrollview setMinimumZoomScale:self.imageView.frame.size.height / MaskCircleSize < 1 ? self.imageView.frame.size.height / MaskCircleSize:1];
    [self.scrollview setContentSize:CGSizeMake(self.imageView.frame.size.width + 30, ScreenHeight)];
    [self.imageView setCenterY:ScreenHeight / 2];
    [self.imageView setCenterX:(self.imageView.frame.size.width + 30) / 2 + 20];
...
- (UIImage *)getSubImage
{
//    UIGraphicsBeginImageContextWithOptions(CGSizeMake(MaskCircleSize,MaskCircleSize), YES, [UIScreen mainScreen].scale);
...
-(void)SaveImageToServer{
    NSData *file = UIImagePNGRepresentation(self.image);
