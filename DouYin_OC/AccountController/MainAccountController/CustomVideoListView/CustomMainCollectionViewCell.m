//
//  CustomMainCollectionViewCell.m
//  DouYin(OC)
//
//  Created by 孙志雄 on 2022/11/29.
//

#import "CustomMainCollectionViewCell.h"
#import "VideoListCollectionView.h"

@interface CustomMainCollectionViewCell() <UIScrollViewDelegate>
...
@property (nonatomic,strong) VideoListCollectionView *view3;
@property (nonatomic,strong) VideoListCollectionView *view4;

@end


@implementation CustomMainCollectionViewCell
...
- (instancetype)initWithFrame:(CGRect)frame{
    if(self = [super initWithFrame:frame]){
        ...
        _view3 = [[VideoListCollectionView alloc] initWithFrame:CGRectMake(ScreenWidth * 2, 0, ScreenWidth,self.frame.size.height)];
        
        
        _view4 = [[VideoListCollectionView alloc] initWithFrame:CGRectMake(ScreenWidth * 3, 0, ScreenWidth,self.frame.size.height)];
...
-(void)initWithData:(NSInteger)currentpageIndex MyOwnVideoList:(NSMutableArray<NSString*>*) MyOwnVideoList MyLoackVideoList:(NSMutableArray<NSString*>*) MyLoackVideoList MyStarVideoList:(NSMutableArray<NSString*>*) MyStarVideoList MyLikeVideoList:(NSMutableArray<NSString*>*) MyLikeVideoList{
    ...
    self.MyStarVideoList = MyStarVideoList;
    self.MyLikeVideoList = MyLikeVideoList;
    [_view1 SetData:self.MyOwnVideoList];
    [_view2 SetData:self.MyLoackVideoList];
    [_view3 SetData:self.MyStarVideoList];
    [_view4 SetData:self.MyLikeVideoList];
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if(translatedPoint.x < -50 && self.currentPageIndex < 3){
...
-(void)UpdateSrollFrame{
    ...
    switch (self.currentPageIndex) {
        case 1:
            ...
            break;
        case 2:
            _view3.frame = CGRectMake(ScreenWidth * 2, 0, ScreenWidth, self.scrollview.frame.size.height);
            [_view3 UpdateFrame:self.scrollview.frame.size.height];
            break;
        case 3:
            _view4.frame = CGRectMake(ScreenWidth * 3, 0, ScreenWidth, self.scrollview.frame.size.height);
            [_view4 UpdateFrame:self.scrollview.frame.size.height];
            break;
        default:
